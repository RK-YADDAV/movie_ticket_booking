      ...
      <img src={assets.logo} alt="" className='w-36 h-auto'/>
        ...
        <div className='flex items-centre gap-8'>
            <SearchIcon className='max-md:hidden w-6 h-6 cursor-pointer'/>
                {
                    !user ? (
                        <button onClick={openSignIn} className='px-4 py-2 text-xs bg-red-600 hover:bg-red-700 
                        transition rounded-full font-medium text-white cursor-pointer'>Login</button>
                    ):(
                        <UserButton>
                            <UserButton.MenuItems>
                                <UserButton.Action label='My Bookings' labelIcon=
                                {<TicketPlus width={15}/>} onClick={()=>
